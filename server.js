...
app.delete('Transaccion/eliminar', (req, res) => {
    const { id } = req.body;
    const borrarquery = 'DELETE FROM Transaccion WHERE Id = ?';
    if (!id) {
        return res.status(400).json({ error: 'Id de  Transaccion no Proporcionada' });
    }
    connection.query(borrarquery, [id], (error, results) => {
        if (error) {
            return res.status(500).json({ error: 'Error al eliminar Transaccion' });
        }
...
app.post('/Transaccion/agregar', (req, res) => {
    const { Tipo_Transaccion, Monto, Cuenta_Origen_Id, Cuenta_Destino_Id, Ficha_Transaccion } = req.body;

    // Consulta para verificar si ya existe una transacción con la misma ficha
    const verificarTransferenciaQuery = 'SELECT * FROM Transaccion WHERE Ficha_Transaccion = ?';
    connection.query(verificarTransferenciaQuery, [Ficha_Transaccion], (err, results) => {
        if (err) {
            console.error('Error al verificar la transacción:', err.message);
            res.status(500).send('Error al verificar la transacción');
            return;
        }
        if (results.length > 0) {
            console.log('La transacción ya está registrada como duplicada');
            res.status(409).json({ message: 'Ya está registrada una transacción con este ID' });
        } else {
            // Si no existe, agregamos la transacción
            const agregarTransaccionQuery = 'INSERT INTO Transaccion (Tipo_Transaccion, Monto, Cuenta_Origen_Id, Cuenta_Destino_Id, Ficha_Transaccion) VALUES (?,?,?,?,?)';

            connection.query(agregarTransaccionQuery, [Tipo_Transaccion, Monto, Cuenta_Origen_Id, Cuenta_Destino_Id, Ficha_Transaccion], (err, results) => {
                if (err) {
                    console.error('Error al agregar la transacción:', err.message);
                    res.status(500).send('Error al agregar la transacción');
                } else {
                    res.status(200).json({
                        message: 'Transacción agregada exitosamente',
                        transaccionId: results.insertId
                    });
                }
            });
        }
